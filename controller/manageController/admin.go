...
func AdminDetailForm(ctx iris.Context) {
	...
	currentSite.AddAdminLog(ctx, ctx.Tr("更新管理员信息：%d => %s", admin.Id, admin.UserName))
...
func AdminDetailDelete(ctx iris.Context) {
	...
	currentSite.AddAdminLog(ctx, ctx.Tr("删除管理员：%d => %s", req.Id, req.UserName))
...
func AdminGroupDetailForm(ctx iris.Context) {
	...
	currentSite.AddAdminLog(ctx, ctx.Tr("更新管理员组信息：%d => %s", req.Id, req.Title))
...
func AdminGroupDelete(ctx iris.Context) {
	...
	currentSite.AddAdminLog(ctx, ctx.Tr("删除管理员组：%d => %s", req.Id, req.Title))
...
func FindPasswordReset(ctx iris.Context) {
	...
	currentSite.AddAdminLog(ctx, ctx.Tr("重置管理员账号和密码：%d => %s", admin.Id, admin.UserName))

	ctx.JSON(iris.Map{
		"code": config.StatusOK,
		"msg":  ctx.Tr("管理员账号和密码已重置"),
	})
}
